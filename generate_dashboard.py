#!/usr/bin/env python3
"""
GTH Dashboard Generator - Lee datos.json del repo y publica index.html via API
"""
import os, json, base64, datetime, urllib.request, urllib.error
from pathlib import Path

def github_get(path, token, repo="GTHHotelero/gth-dashboard", branch="main"):
...
def github_put(path, content_bytes, token, sha=None, repo="GTHHotelero/gth-dashboard", branch="main"):
    ...
    body = {"message": f"Dashboard GTH · {datetime.date.today().strftime('%d/%m/%Y')} · automático", "content": base64.b64encode(content_bytes).decode(), "branch": branch}
    if sha: body["sha"] = sha
...
def generate_html(db, logo_b64):
    ...
    fechas_opts = "".join([f'<option value="{f}"{" selected" if f==ultima else ""}>{"/".join(reversed(f.split("-")))}</option>' for f in sorted(db["fechas"], reverse=True)])

    CSS = """:root{--gold:#9E8E5A;--grey:#787878;--grey-bg:#F4F3F0;--grey-d:#3A3A3A;--bg:#F9F8F5;--card:#fff;--plr:#378ADD;--hjc:#1D9E75;--soho:#D85A30;--red:#C0392B;--green:#1D9E75;--border:#E8E5DE;--sh:0 2px 12px rgba(0,0,0,0.07)}
...
.date-bar{background:#fff;border-bottom:1px solid #eee;padding:8px 28px;display:flex;align-items:center;gap:12px}
.date-select{font-family:'DM Sans',sans-serif;font-size:13px;color:var(--grey-d);background:#fff;border:1px solid #e8e5de;border-radius:6px;padding:6px 12px;cursor:pointer}
...
.tab-btn{padding:14px 22px;border:none;background:none;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:500;color:var(--grey);cursor:pointer;border-bottom:3px solid transparent;transition:all .2s}
...
.section-title{font-family:'Playfair Display',serif;font-size:15px;font-weight:600;margin-bottom:16px;padding-bottom:6px;border-bottom:1px solid #e8e5de;display:flex;align-items:center;gap:8px}
...
.badge-dia{background:rgba(158,142,90,.15);color:var(--gold);font-size:9px;padding:1px 5px;border-radius:4px;font-weight:700}
...
.rev-mini-value{font-size:14px;font-weight:700}
...
.chart-title{font-size:13px;font-weight:600;margin-bottom:16px;padding-bottom:8px;border-bottom:1px solid #f0ede6}
...
function renderAll(fecha){{fechaActual=fecha;const fd=fecha.split("-");document.getElementById("fecha-header").textContent=fd[2]+"/"+fd[1]+"/"+fd[0];renderAlertas(fecha);renderResumen(fecha);renderHoteles(fecha);renderSeg(fecha);if(document.getElementById("tab-graficos").classList.contains("active"))renderGraficos(fecha);}}
function showTab(name,event){{document.querySelectorAll(".tab-panel").forEach(p=>p.classList.remove("active"));document.querySelectorAll(".tab-btn").forEach(b=>b.classList.remove("active"));document.getElementById("tab-"+name).classList.add("active");if(event)event.target.classList.add("active");if(name==="graficos")renderGraficos(fechaActual);}}
...
<div class="topbar">
  <div class="topbar-logo">
    ...
    <div><div class="topbar-title">Dashboard Hotelero</div><div style="font-size:11px;color:var(--grey)">Reporte del día · K007</div></div>
  </div>
  <div class="topbar-meta"><strong id="fecha-header">{fecha_display}</strong><br>3 propiedades · 267 habitaciones</div>
</div>
<div class="date-bar">
  <span style="font-size:11px;color:var(--grey);font-weight:600">VER FECHA:</span>
  <select class="date-select" id="date-select" onchange="renderAll(this.value)">{fechas_opts}</select>
</div>
...
<div class="main">
  <div id="tab-resumen" class="tab-panel active"><div class="section-title"><span class="dot"></span>Cadena GTH — Consolidado</div><div id="resumen-cadena"></div></div>
  <div id="tab-hotel" class="tab-panel"><div id="hotel-sections"></div></div>
  <div id="tab-segmentos" class="tab-panel">
    <div class="seg-table-wrap"><table class="seg-table"><thead><tr><th style="text-align:left">Segmento</th><th>Mes actual</th><th>Mes ant.</th><th>Var %</th><th>Año ant.</th><th>Var AA%</th></tr></thead><tbody id="seg-tbody"></tbody></table></div>
    <div class="chart-card" style="margin-top:16px"><div class="chart-title">Segmentos por hotel</div><canvas id="chartSeg" height="110"></canvas></div>
  </div>
...
if __name__ == "__main__":
    ...
    # Leer datos.json desde GitHub
    print("Leyendo datos.json desde GitHub...", flush=True)
    try:
        datos_text, datos_sha = github_get("datos.json", gh_token)
        db = json.loads(datos_text)
        print(f"DB OK: {len(db['fechas'])} fechas, ultima: {db['ultima_fecha']}", flush=True)
    except Exception as e:
        print(f"Error leyendo datos.json: {e}", flush=True)
        sys.exit(1)
    ...
    print("Subiendo index.html a GitHub...", flush=True)
    # Obtener SHA del index.html actual
    try:
        _, idx_sha = github_get("index.html", gh_token)
    except:
        idx_sha = None
    github_put("index.html", html.encode("utf-8"), gh_token, sha=idx_sha)
